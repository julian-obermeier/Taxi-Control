...
class InstallerController extends Controller
{
    ...
    public function install(Request $request): RedirectResponse
    {
        ...
        try {
            if (file_put_contents(base_path('.env'), $this->buildEnv($data, $appKey), LOCK_EX) === false) {
            ...
            @unlink(storage_path('app/.preinstall-key'));
            ...
            return redirect()->route('taxi-control.superadmin.dashboard')
                ->with('status', 'Taxi-Control wurde erfolgreich installiert. Bitte aktivieren Sie jetzt 2FA für den Superadministrator.');
        } catch (Throwable $e) {
            report($e);
